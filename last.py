...
# Define a SongRecognition model for storing results
...
# Create the database tables (run this once to create the table)
...
def store_result_in_json(song_name, file_name, folder_path):
    result_data = {
        'song_name': song_name,
        'file_name': file_name,
        'folder_path': folder_path,
        'recognized_at': str(datetime.utcnow())
    }

    # Append the result to a JSON file
    if os.path.exists('recognition_results.json'):
        with open('recognition_results.json', 'r+') as file:
            data = json.load(file)
            data.append(result_data)
            file.seek(0)
            json.dump(data, file, indent=4)
    else:
        with open('recognition_results.json', 'w') as file:
...
def recognize():
    ...
    try:
        ...
        store_result_in_json(recognized_song, file.filename, folder_path)
