...
#                 except Exception as e:
#                     # Log or handle errors specific to feature extraction or metadata
...
#         metadata = metadata_dict.get(most_similar_song, {})
#         result = {
#             'message': f"The song most similar to the uploaded file is: {metadata.get('title', 'Unknown Title')} by {metadata.get('artist', 'Unknown Artist')} from the album {metadata.get('album', 'Unknown Album')}.",
#             'metadata': metadata,
#             'distances': distances
#         }
#     else:
...
@app.route('/')
def index():
    return render_template('inde.html')  # Ensure this file exists in the templates folder
